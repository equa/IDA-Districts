...
class CopySupervisoryControl:
    ...
    def __init__(self,dir,dictDB,cur,submodel):
        ...
        copyNestedSupervisoryMacros(source_dir+'\\Supervisory_control\\Supervisory_control',target_dir+'\\Supervisory_control')

class Supervisory_control():
    def __init__(self,plugin_dir,update_sensors=False):
        ...
        if self.conn:
            ...
            createDir(plugin_dir+'\\models',self.dictDB['projectName'])
            dir=plugin_dir+'\\models\\'+self.dictDB['projectName']
            ...
            #get sensor data
            sensor_data_source=getSensorData(self.cur,self.dictDB,source_types=[3])
            sensor_data_target=getSensorData(self.cur,self.dictDB,target_types=[3])
            ...
            if update_sensors:
                ...
                remove_sensor_target_ids=getRemovedSensorTargetData(self.cur,self.dictDB,target_types=[3])
...
((MACRO-OBJECT :N "Supervisory_control" :T ICE-MACRO :ETM 3857461881 :STM 3857461887){})
((MACRO-OBJECT :N "Climate-macro" :T ICE-MACRO :ETM 3857461881 :STM 3857461887))
((MACRO-OBJECT :N "Sensor-macro" :T ICE-MACRO :ETM 3857461881 :STM 3857461887){})
(CONNECTIONS {})""".format(supervisory_conns,sensor_conns,connections)
...
(SELF-FRAME :AT ((352 190)) :R (342 176) :SLOT (:SELF) :DATA MACRO-OBJECT)\n{}{}""".format(
            ...
            #climate macro
            writeMacroClimateIdm(self.dictDB,self.cur,'supervisory_control','\\'.join(dir.split("\\")[:-1]),plugin_dir,loadModellingSettings(plugin_dir,self.dictDB),getClimateData(self.cur,self.dictDB,True))
            writeMacroClimateIdc('supervisory_control','\\'.join(dir.split("\\")[:-1]),loadModellingSettings(plugin_dir,self.dictDB))
