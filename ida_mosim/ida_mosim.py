...
from .ida_mosim_dialog import BuildBuildingModelDialog, LoadResultsDialog,CalibrateCustomers, OpenModelDialog,SupervisoryCtrlDlg, FeatureDecouplingDlg, FeatureModelParmDlg, ModellingSettings, IDADistrictsModelingSimulationDialog, RequestedOutputsDialog, BuildNetworkModelDialog, CheckableComboBox, RunNetworkModelDialog
...
class IDADistrictsModelingSimulation:
    ...
    def openSupervisoryCtrl(self):
        ...
        file = self.plugin_dir+'\\models\\{}\\{}\\supervisory_control\\supervisory_control.idm'.format(self.dictDB['projectName'],self.dictDB['versionName'])
        file=file.replace('/','\\')
        print(file)
        self.worker_openSupervisory = WorkerOpenModelCmd(file,self.plugin_dir)
    ...
    def openModel(self,dlg,mode='network'):
        ...
        for i in range(dlg.combo_submodels.count()):
            if dlg.combo_submodels.itemText(i) != 'Check all items' and dlg.combo_submodels.itemChecked(i):        
                ...
                self.worker_openNetwork = WorkerOpenModelCmd(fname,self.plugin_dir,submodel=submodel)
            ...
            if self.checkSimOutputs():
                ...
                for i in range(dlg.combo_submodels.count()):
                    if dlg.combo_submodels.itemText(i) != 'Check all items' and dlg.combo_submodels.itemChecked(i):        
                        ...
                        self.worker_runNetwork[i].signals.finished.connect(dlg.update_finished)   
            else:
...
(set-values [hc_ SHAPE] 'ncorn ncorn 'CORNERS corn-array)\n""".format(zone['b_id'],zone['b_id'])
  ...
  (move-internal-gain-to-center load))\n"""
                        ...
                        print(src_dir)
                        buildingModel_dir=self.plugin_dir+'\\models\\{}\\{}\\'.format(self.dictDB['projectName'],self.dictDB['versionName'])
    ...
    def finishedBuildBuildingModel(self,args):
        ...
        try:
            ...
            print(submodel)    
            print(conn_data) 
            source_dir=self.plugin_dir+'\\models\\{}\\{}\\invoked_customers\\'.format(self.dictDB['projectName'],self.dictDB['versionName'])
            target_dir=self.plugin_dir+'\\models\\{}\\{}\\building_{}\\plant\\'.format(self.dictDB['projectName'],self.dictDB['versionName'],submodel)
            print(source_dir) 
    ...
    def showRequestedOutputs(self):
        ...
        if self.conn:
            if self.dictDB['versionName']:
                ...
                self.dlg_outputs.btn_ok.clicked.connect(lambda: self.setRequestedOutputs(self.dlg_outputs,self.requestedOutputs))
    ...
    def setRequestedOutputs(self,dlg,requestedOutputs):
        """set requested outputs"""
        print("set requested outputs")
        self.worker_setRequestedOutputs = WorkerSetRequestedOutputs(dictDB=self.dictDB,plugin_dir=self.plugin_dir,dlg=dlg,requestedOutputs=requestedOutputs)
        self.threadpool_requestedOutputs = QThreadPool()
        self.threadpool_requestedOutputs.start(self.worker_setRequestedOutputs) 
        self.worker_setRequestedOutputs.signals.error.connect(show_error_message)
        self.worker_setRequestedOutputs.signals.progress.connect(dlg.update_progress)   
        self.worker_setRequestedOutputs.signals.finished.connect(dlg.update_finished)  
