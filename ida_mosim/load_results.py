...
(
    ...
    ORDER BY f.id;""".format(self.dictDB['versionName'],submodel_id)
                        self.cur.execute(sql)
...
(
    ...
    WHERE b_t_conns.conn_bundle_type_id = t.conn_bundle_type AND f.template=t.template AND submodel={};""".format(self.dictDB['versionName'],submodel_id)
                        print(sql)
                        self.cur.execute(sql)
...
(
	...
	CONSTRAINT line_s_{}${}_pkey PRIMARY KEY (id)
                        ...
                        #load data
                        #get lids
                        sql="""SELECT id FROM "{}".lines WHERE {}=ANY(submodel) ORDER BY id;""".format(self.dictDB['versionName'],submodel_id)
                        self.cur.execute(sql)
