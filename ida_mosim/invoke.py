...
def addParmTableRow(dlg,cur,dictDB):
    ...
    maxId=max(getMaxIdAcrossSchemas(dictDB,cur,'model_parms')+1,max([int(dlg.tableWidget_parameters.item(i,0).text())+1 for i in range(dlg.tableWidget_parameters.rowCount())],default=0))
...
def invokeOneFeature(dlg,idx,plugin_dir,cur,dictDB,type,invoked,parmRun=False,saveParmRunResults=False,parallize=False,signals=False):
    ...
    if idx !=-1:
        ...
        dir=plugin_dir+"\\models\\{}\\{}\\invoked_{}s\\".format(dictDB['projectName'],dictDB['versionName'],type)
        #print(dir)
        ...
        for feature in cur.fetchall():
            ...
            if type=='customer':
                ...
                if dhw:
                    ...
                    dir_dhw=plugin_dir+'\\models'+'\\'+dictDB['projectName']+'\\'+dictDB['versionName']
                ...
                if internal_load:
                    ...
                    dir_internal_loads=plugin_dir+'\\models'+'\\'+dictDB['projectName']+'\\'+dictDB['versionName']
...
class InvokeFeatures():
    ...
    def updateFeatureLists(self,s):
        ...
        if ids:
            for id in ids:
                ...
                if os.path.exists(self.plugin_dir+"\\models\\{}\\{}\\invoked_{}s\\{}_{}.idm".format(self.dictDB['projectName'],self.dictDB['versionName'],self.type,self.type.capitalize(),id)): 
                    exists='Yes'
                else:
                    exists='No'
    ...
    def loadInvokeFeatureData(self,dlg):
        ...
        for id in self.cur.fetchall():
            ...
            if os.path.exists(self.plugin_dir+"\\models\\{}\\{}\\invoked_{}s\\{}_{}.idm".format(self.dictDB['projectName'],self.dictDB['versionName'],self.type,self.type.capitalize(),str(id['id']))): 
                exists='Yes'
            else:
                exists='No'
    ...
    def openInvokedFeature(self,dlg):
        ...
        if idx!=-1:
            id=dlg.tableWidget_customer.item(idx,0).text()
            file_path=self.plugin_dir+"\\models\\{}\\{}\\invoked_{}s\\{}_{}.idm".format(self.dictDB['projectName'],self.dictDB['versionName'],self.type,self.type.capitalize(),id)  
    ...
    def simulateInvokedFeatures(self,dlg):
        ...
        #print(ids)
        if ids:
            file_pathes=[self.plugin_dir.replace('/','\\')+"\\models\\{}\\{}\\invoked_{}s\\{}_{}.idm".format(self.dictDB['projectName'],self.dictDB['versionName'],self.type,self.type.capitalize(),i)  for i in ids]
            #print(file_pathes)
