...
def getResultSensorData(cur,config,execute_query=True,source_types=[1,2],network=1):
    sql="""WITH raw_sensor_features AS (
    -- 1. customer (type:1); temp,mass,p(measure:1,2,3) 
    SELECT 
        s.sensor_id, 
        f.id::text AS feature_id, 
        b_t_conns.conn_bundle_type_id::text, 
        b_t_conns.conn_type_id::text, 
        c_t_conns.connection_id::text,
        1 AS source_type
    FROM "{}".customers f
    INNER JOIN public.customer_templates f_t ON f.template = f_t.template
    INNER JOIN public.bundle_type_conns b_t_conns ON b_t_conns.conn_bundle_type_id = f_t.conn_bundle_type
    INNER JOIN public.connection_type_connections c_t_conns ON c_t_conns.connection_type_id = b_t_conns.conn_type_id
    INNER JOIN public.sensor_source s ON s.type = 1
    INNER JOIN public.source_template st ON st.source_id = s.sensor_id AND st.template = f.template AND st.active = true
    INNER JOIN public.source_conn_type s_ct ON s_ct.source_id = s.sensor_id AND s_ct.conn_type = b_t_conns.conn_type_id AND s_ct.active = true
    INNER JOIN public.source_conns s_c ON s_c.source_id = s.sensor_id AND s_c.connection_id = c_t_conns.connection_id AND s_c.active = true
    INNER JOIN "{}".customer_connections fc ON fc.cid = f.id AND fc.c_seq = b_t_conns.sequence
    INNER JOIN "{}".lines l ON fc.lid = l.id AND l.network = {}
    INNER JOIN public.bundle_pipes bp ON bp.pipe_bundle_type_id = l.pipe_bundle_type_id AND bp.sequence = fc.c_seq
    WHERE {} = ANY(f.network)

    UNION ALL

    -- 2. plant (type:2); temp,mass,p(measure:1,2,3) 
    SELECT 
        s.sensor_id, 
        f.id::text AS feature_id, 
        b_t_conns.conn_bundle_type_id::text, 
        ...
        c_t_conns.connection_id::text,
        2 AS source_type
    FROM "{}".energy_plants f
    INNER JOIN public.energy_plant_templates f_t ON f.template = f_t.template
    INNER JOIN public.bundle_type_conns b_t_conns ON b_t_conns.conn_bundle_type_id = f_t.conn_bundle_type
    INNER JOIN public.connection_type_connections c_t_conns ON c_t_conns.connection_type_id = b_t_conns.conn_type_id
    INNER JOIN public.sensor_source s ON s.type = 2
    INNER JOIN public.source_template st ON st.source_id = s.sensor_id AND st.template = f.template AND st.active = true
    INNER JOIN public.source_conn_type s_ct ON s_ct.source_id = s.sensor_id AND s_ct.conn_type = b_t_conns.conn_type_id AND s_ct.active = true
    INNER JOIN public.source_conns s_c ON s_c.source_id = s.sensor_id AND s_c.connection_id = c_t_conns.connection_id AND s_c.active = true
    INNER JOIN "{}".energy_plant_connections fc ON fc.epid = f.id AND fc.ep_seq = b_t_conns.sequence
    INNER JOIN "{}".lines l ON fc.lid = l.id AND l.network = {}
    INNER JOIN public.bundle_pipes bp ON bp.pipe_bundle_type_id = l.pipe_bundle_type_id AND bp.sequence = fc.ep_seq
    WHERE {} = ANY(f.network)

    UNION ALL

    -- 3. customer (type:1); power(measure:4) 
    SELECT 
        s.sensor_id, 
        f.id::text AS feature_id, 
        b_t_conns.conn_bundle_type_id::text, 
    ...
    INNER JOIN public.customer_templates f_t ON f.template = f_t.template
    INNER JOIN public.bundle_type_conns b_t_conns ON b_t_conns.conn_bundle_type_id = f_t.conn_bundle_type
    INNER JOIN public.sensor_source s ON s.type = 1 AND s.measure = 4
    INNER JOIN public.source_template st ON st.source_id = s.sensor_id AND st.template = f.template AND st.active = true
    INNER JOIN public.source_conn_type s_ct ON s_ct.source_id = s.sensor_id AND s_ct.conn_type = b_t_conns.conn_type_id AND s_ct.active = true
    INNER JOIN "{}".customer_connections fc ON fc.cid = f.id AND fc.c_seq = b_t_conns.sequence
    INNER JOIN "{}".lines l ON fc.lid = l.id AND l.network = {}
    ...
    -- 4. plant (type:2); power(measure:4) 
    SELECT 
        s.sensor_id, 
        f.id::text AS feature_id, 
        b_t_conns.conn_bundle_type_id::text, 
    ...
    INNER JOIN public.energy_plant_templates f_t ON f.template = f_t.template
    INNER JOIN public.bundle_type_conns b_t_conns ON b_t_conns.conn_bundle_type_id = f_t.conn_bundle_type
    INNER JOIN public.sensor_source s ON s.type = 2 AND s.measure = 4
    INNER JOIN public.source_template st ON st.source_id = s.sensor_id AND st.template = f.template AND st.active = true
    INNER JOIN public.source_conn_type s_ct ON s_ct.source_id = s.sensor_id AND s_ct.conn_type = b_t_conns.conn_type_id AND s_ct.active = true
    INNER JOIN "{}".energy_plant_connections fc ON fc.epid = f.id AND fc.ep_seq = b_t_conns.sequence
    INNER JOIN "{}".lines l ON fc.lid = l.id AND l.network = {}
...
distinct_sensor_features AS (
    -- Liefert eindeutige Rohdaten-Zeilen zurück
    SELECT 
        r.sensor_id, 
        s.function, 
        ...
        r.conn_type_id, 
        r.connection_id
    FROM raw_sensor_features r
    INNER JOIN public.sensor_source s ON s.sensor_id = r.sensor_id
    GROUP BY r.sensor_id, s.function, r.source_type, r.feature_id, r.conn_type_id, r.connection_id
),
at_names_mapping AS (
    SELECT 1 AS type, f.id::text AS feature_id, '1:' || f.template::text || '_' || ct.template_name AS template_name 
    FROM "{}".customers f
    INNER JOIN public.customer_templates ct ON ct.template = f.template
    UNION ALL
    SELECT 2 AS type, f.id::text AS feature_id, '2:' || f.template::text || '_' || et.template_name AS template_name 
    FROM "{}".energy_plants f
    INNER JOIN public.energy_plant_templates et ON et.template = f.template
),
target_mapping AS (
    SELECT 
        t.sensor_id, 
...
-- FINALE SELEKTION MIT KORREKTER AGGREGATION
SELECT 
    sub.sensor_id, 
    s.type AS source_type, 
    type2.name AS source_type_name, 
    -- Hier nutzen wir ARRAY_AGG(DISTINCT ...), um alle gefundenen Templates in geschweifte Klammern zu packen:
    ARRAY_AGG(DISTINCT m_map.template_name) AS source_template_names, 
    t.type AS target_type, 
    type1.name AS target_type_name, 
    -- Hier packen wir "results" in ein Array, um exakt der Soll-Struktur zu entsprechen:
    ARRAY_AGG(DISTINCT t_map.target_template_name) AS target_template_names,
    sub.function, 
    s_f.function AS function_name, 
    s.measure, 
...
FROM distinct_sensor_features sub
INNER JOIN public.sensor_source s ON s.sensor_id = sub.sensor_id AND s.type IN ({})
INNER JOIN public.sensor_target t ON t.sensor_id = sub.sensor_id AND t.type = 4
INNER JOIN public.signal_function s_f ON s_f.id = s.function
INNER JOIN public.measure m ON m.id = s.measure 
INNER JOIN public.type type1 ON type1.id = t.type 
INNER JOIN public.type type2 ON type2.id = s.type
LEFT JOIN at_names_mapping m_map ON m_map.type = sub.type AND m_map.feature_id = sub.feature_id
LEFT JOIN target_mapping t_map ON t_map.sensor_id = sub.sensor_id
GROUP BY 
    sub.sensor_id, 
    s.type, 
    type2.name, 
