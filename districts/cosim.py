...
(DOCUMENT-HEADER :TYPE ICE-MACRO :D "ICE macro" :APP (ICE :VER 5.19001)) \n"""]
    ...
    for template in usedDecoupledFeatureTemplates:
        print(template)
        #collect resources
        resource_f=plugin_dir+'\\projects\\{}\\{}_templates\\{}_{}.idm'.format(dictDB['projectName'],template['feature'],template['template'],template['template_name'])
        file_data=readFileToList(resource_f)
        resource=getResourcesFromFileDataList(file_data)
        if resource not in resources:
            resources+=resource
            
        template_dir=datacenter_dir+'\\projects\\{}\\{}_templates\\{}_{}\\'.format(dictDB['projectName'],template['feature'],template['template'],template['template_name'])
