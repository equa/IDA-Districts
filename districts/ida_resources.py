...
from .utility_functions.workers import WorkerOpenModelCmd
from .utility_functions.invoke import CopyTemplateFiles
from .utility_functions.layer_visualization import *

def writeRenameExchangeConntype(config,cur,plugin_dir,dlg,traceValue,table_name):
    wroteTemplate=False
        ...
        if dlg.traceTableValues[traceValue][0]!=dlg.traceTableValues[traceValue][1]:
            ...
            if not dlg.traceTableValues[traceValue][0]:
                if dlg.traceTableValues[traceValue][3]:
                    WriteTemplateFiles(config,plugin_dir,dlg.traceTableValues[traceValue][1],table_name,cur,dlg.traceTableValues[traceValue][3])
                else:
                    iface.messageBar().pushMessage("Error", "No template is set in: "+str(dlg.traceTableValues[traceValue]), level=Qgis.Critical)
                    return False
                wroteTemplate=True
            else:
                RenameTemplateFiles(config,plugin_dir,dlg.traceTableValues[traceValue][0],table_name,dlg.traceTableValues[traceValue][1],cur)
    if dlg.traceTableValues[traceValue][3] and dlg.traceTableValues[traceValue][2] and not wroteTemplate:
        if dlg.traceTableValues[traceValue][2]!=dlg.traceTableValues[traceValue][3]:
            ...
            ExchangeConntypeFiles(plugin_dir,name,table_name,dlg.traceTableValues[traceValue][3],dlg.traceTableValues[traceValue][2],cur)
  
def saveAsTemplate(cur,config,plugin_dir,dlg,table_name,dropdowns,trace):
    ...
    if row_idx!=-1:
        ...
        if not os.path.exists(plugin_dir+"\\projects\\{}\\{}\\{}.idm".format(config['projectName'],table_name,file_name_new)): 
            CopyTemplateFiles(source_dir=plugin_dir+"\\projects\\{}\\{}".format(config['projectName'],table_name),source_name=file_name_old,target_dir=plugin_dir+"\\projects\\{}\\{}".format(config['projectName'],table_name),target_name=file_name_new)
...
def openTemplate(cur,config,plugin_dir,type,dlg):
    ...
    if row_index!=-1:
        ...
        dir=plugin_dir.replace('/','\\')+"\\projects\\{}\\{}_templates\\".format(config['projectName'],type)
        ...
        if dlg.traceTableValues[row_index][1] and dlg.traceTableValues[row_index][0]:
            if dlg.traceTableValues[row_index][0]!=dlg.traceTableValues[row_index][1]:
                print('~~~~~~~~~~~~~~~rename~~~~~~~~~~')
                RenameTemplateFiles(config,plugin_dir,dlg.traceTableValues[row_index][0],type,dlg.traceTableValues[row_index][1],cur)
        ...
        if not os.path.exists(file): 
            WriteTemplateFiles(config,plugin_dir,name,type,cur,conn_bundle_type)
            dlg.traceTableValues[row_index][0]=dlg.traceTableValues[row_index][1]
            dlg.traceTableValues[row_index][1]=''
            wroteTemplate=True
        ...
        if dlg.traceTableValues[row_index][3] and dlg.traceTableValues[row_index][2] and not wroteTemplate: # override connection bundle only if template exists before
            if dlg.traceTableValues[row_index][2]!=dlg.traceTableValues[row_index][3]:
                ...
                ExchangeConntypeFiles(plugin_dir,name,type,dlg.traceTableValues[row_index][3],dlg.traceTableValues[row_index][2],cur)
        ...
        sql="""INSERT INTO public.{}_templates (id, template,template_name,conn_bundle_type,description) VALUES({},{},'{}',{},'{}');\n""".format(
            type,getMaxId(cur,type+'_templates')+1,template,template_name,dlg.traceTableValues[row_index][3] if dlg.traceTableValues[row_index][3] else dlg.traceTableValues[row_index][2],description)
        print(sql)
        cur.execute(sql)
        ...
        print('**********************************')
        print(file)
        worker_openTemplate = WorkerOpenModelCmd(file,plugin_dir,config)
        threadpool_openTemplate = QThreadPool()
        threadpool_openTemplate.start(worker_openTemplate)
...
def show_templateDialog(main=False,type=False):
    ...
    dlg.btn_open.clicked.connect(lambda: openTemplate(main.cur,main.config,main.plugin_dir,type,dlg))
    ...
    if save_as:
        dlg.btn_saveAs.clicked.connect(lambda: saveAsTemplate(main.cur,main.config,main.plugin_dir,dlg,table,dropdowns,trace))
    ...
    if trace in ['conn_type_trace','bt_conns_trace']:
        ...
        if [True for traceValue in dlg.traceTableValues if 
            ...
            for template in templates:
                ...
                ExchangeConntypeFiles(plugin_dir,config,template['t_name'],template['type'],template['conn_bundle_type_id'],template['conn_bundle_type_id'],cur,oldConnValues=oldConnValues_dict[template['t_name']])
        
    elif trace:
        print(dlg.traceTableValues)
        for traceValue in dlg.traceTableValues:
            writeRenameExchangeConntype(config,cur,plugin_dir,dlg,traceValue,table_name)
        
        #delete templates in directory if they are not in dlg.traceTableValues[traceValue][0]
        dir=plugin_dir+'\\projects\\'+config['projectName']+"\\{}_templates".format(table_name)
        files = []
...
def show_TableCurrentRowDialog(main,table,columns,dropdowns,dlg,id,openFnArg,trace=False):
    ...
    if id!=-1:
        ...
        if save_as:
            dlg.btn_saveAs.clicked.connect(lambda: saveAsTemplate(dlg,openFnArg[0],dropdowns,trace,main.cur,main.plugin_dir,main.config))
...
def saveConnectionsTable(dlg,table,columns,dropdowns,openFnArg,checkBoxes,main):
    ...
    if checkConnsInputValues(dlg):
        updateTemplateBoundaryValues(dlg,main.plugin_dir,main.config,main.cur)
        saveTable(dlg,table,columns,dropdowns,openFnArg,checkBoxes,False,False,main=main)
