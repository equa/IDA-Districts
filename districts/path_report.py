...
SELECT ep.id, ST_Z(ST_EndPoint(l.geom)) AS height, 'energy_plant' AS type
    ...
    GROUP BY ep.id,l.geom;""".format(self.config['versionName'],self.config['versionName'],self.config['versionName'],conn_type_seq,lid,
    self.config['versionName'],self.config['versionName'],self.config['versionName'],conn_type_seq,lid)
        #print(sql)
    ...
    GROUP BY ep.id,l.id, height,l.geom;""".format(self.config['versionName'],self.config['versionName'],self.config['versionName'],epid,conn_type_seq)
        #print(sql)
        self.cur.execute(sql)
        data=self.cur.fetchone()
    ...
    GROUP BY ep.id,l.id, height,l.geom, b_t_conns.conn_type_id;""".format(self.config['versionName'],self.config['versionName'],self.config['versionName'],network,network,epid)
        #print(sql)
        self.cur.execute(sql)
        data=self.cur.fetchone()
        epid=[str(data['epid']),data['height']]
    ...
    WHERE l1.id!=l2.id AND l1.id={} AND l2.id IN ({}) AND ST_dWithIn(ST_EndPoint(l1.geom),l2.geom,0.0001) """.format(self.config['versionName'],self.config['versionName'],lids_new[-1][0],",".join([str(lid) for lid in lids]))
            #print(sql)
            self.cur.execute(sql)
            data=self.cur.fetchone()
...
SELECT pgr_createTopology('temp.streets_help',0.0001,'geom','id',clean:='true');""".format(self.config['versionName'])
                self.cur.execute(sql)
                
                sql="""SELECT st_v.id::integer AS vid FROM temp.streets_help_vertices_pgr st_v,"{}".energy_plants ep WHERE ST_dWithin(ep.geom,st_v.the_geom,0.0001) AND ep.id={};""".format(self.config['versionName'],self.dlg.main_plant.currentText())
                #print(sql)
                self.cur.execute(sql)
...
SELECT sub.fid, sub.dvar, var_f1.time, var_f1."${}" as sup_f, var_f2."${}" as ret_f, ST_Z(var_f1.geom) AS height_f, var_ep1."${}" as sup_ep, var_ep2."${}" as ret_ep, ST_Z(var_ep1.geom) AS height_ep 
    ...
    WHERE var_f1.time=var_ep1.time AND var_ep2.time=var_ep1.time AND var_f2.time=var_ep1.time AND var_ep1.fid={} AND var_ep1.fid=var_ep2.fid AND var_ep1."${}"-var_f1."${}" = sub.dvar AND sub.fid=var_f1.fid AND sub.fid=var_f2.fid{};""".format(
        quantity_var,quantity_var,self.config['versionName'],f_type,quantity_var,p_f_sup_ident,self.config['versionName'],quantity_var,p_ep_sup_ident,
        self.dlg.main_plant.currentText(),' AND var_f.fid IN({})'.format(','.join(fids)) if self.dlg.rbtn_customer.isChecked() or self.dlg.rbtn_energy_plant.isChecked() or self.dlg.rbtn_lineIds.isChecked() else '',
        quantity_var,quantity_var,quantity_var,quantity_var,
        self.config['versionName'],f_type, quantity_var, p_f_sup_ident,self.config['versionName'],f_type,quantity_var,p_f_ret_ident,self.config['versionName'],quantity_var, p_ep_sup_ident,self.config['versionName'],quantity_var, p_ep_ret_ident,
        self.dlg.main_plant.currentText(), quantity_var, quantity_var, ' AND var_f1.fid IN({})'.format(','.join(fids)) if self.dlg.rbtn_customer.isChecked() or self.dlg.rbtn_energy_plant.isChecked() or self.dlg.rbtn_lineIds.isChecked() else '')
            else:
    ...
    ORDER BY dvar DESC
    LIMIT 1;""".format(quantity_var,quantity_var,quantity_var,quantity_var,quantity_var,quantity_var,
        self.config['versionName'],f_type, quantity_var, p_f_sup_ident,self.config['versionName'],f_type, quantity_var, p_f_ret_ident,self.config['versionName'], quantity_var, p_ep_sup_ident,self.config['versionName'], quantity_var, p_ep_ret_ident,
        self.dlg.date_input.text(),self.dlg.main_plant.currentText(), ' AND var_f1.fid IN({})'.format(','.join(fids)) if self.dlg.rbtn_customer.isChecked() or self.dlg.rbtn_energy_plant.isChecked() or self.dlg.rbtn_lineIds.isChecked() else '')
            ...
            if not self.dlg.rbtn_lineIds.isChecked():
                ...
                sql="""SELECT st_v.id::integer AS vid FROM temp.streets_help_vertices_pgr st_v,"{}".customers f WHERE ST_dWithin(f.geom,st_v.the_geom,0.0001) AND f.id={};""".format(self.config['versionName'],self.dlg.weak_point['fid'])
                #print(sql)
                self.cur.execute(sql)
...
SELECT l.id,ST_3DLength(l.geom) AS length FROM sub,temp.streets_help st_h, "{}".lines l WHERE sub.edge=st_h.id AND st_h.geom=st_force2D(l.geom);""".format(v_epid,v_id,self.config['versionName'])
                self.cur.execute(sql)
...
SELECT var1.fid,var1."${}" AS var1, var2."${}" AS var2, ABS(var1."${}" - var2."${}") AS dvar, ST_Z(ST_EndPoint(var1.geom)) AS height_j
    ...
    ORDER BY var1.fid;""".format(
    self.config['versionName'],quantity_var,
    quantity_var,quantity_var,quantity_var,quantity_var,
    self.config['versionName'], quantity_var, self.dlg.sup_sequence.currentText(),self.config['versionName'],quantity_var, self.dlg.ret_sequence.currentText(), 
