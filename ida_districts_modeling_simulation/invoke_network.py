...
class InvokeNetworkModel:
        ...
        if self.conn:
            ...
            for submodel in submodels: 
                ...
                data.append(''.join(["""(:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 224)\n""".format(j['iref']) 
                    ...
                    j['submodel']==submodel and not j['network_side'] or j['cosim']==submodel and j['network_side']]))
                #add irefs Source/targets if source type==Supervisory ctrl (4) and function == Individual signals for each target and target submodel != supervisory submodel
                data.append(''.join(["""(:IREF :N "Int_Ref_Sensor_{}_{}" :T {})\n""".format('Source' if submodel==supervisory_submodel else 'Target',j['iref'],'IN :F 208' if submodel==supervisory_submodel else 'OUT :F 224') 
                                    for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                ...
                #add irefs targets if target type==Supervisory ctrl (4) and function == Individual signals for each target (6) for decoupling at supervisory side
                data.append(''.join(["""(:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 208)\n""".format(j['iref']) 
                                    for i in sensor_dec_data  if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                                    for j in i['irefs_source'] if supervisory_submodel==submodel and submodel!=j['submodel'] and not j['network_side']]))
                #add irefs targets if target type==Supervisory ctrl (4) and function == Individual signals for each target (6) for decoupling at feature side
                data.append(''.join(["""(:IREF :N "Int_Ref_Sensor_Source_{}" :T IN :F 208)\n""".format(j['iref']) 
                                    for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                ...
                #add Adder comp for function Average (3), Add (4), Same signal for all targets (5) if measure in (1,2,3,4) and source type in (1,2,3)
 ...
 (:PAR :N N_IN :V {}))\n""".format(str(i['sensor_id']),
                                    " ".join(["""({} :SYSTEM "{}" "{}" {})""".format(conn[0], getMacroTypeName(i['source_type'])+'_'+conn[1][0].split('_')[1], getPMT2muxName(self.cur,conn[1][1]['conn_bundle_type_id'],conn[1][0].split('_')[3]), returnVarName(i['measure'])) 
                                        for conn in enumerate([[j[1],getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)] for j in i['irefs_source']],1) if i['measure'] in (1,2,3)]),
                                    " ".join(["""({} :SYSTEM "{}" "{}" P)""".format(conn[0], getMacroTypeName(i['source_type'])+'_'+conn[1][0].split('_')[1], getMeterName(self.cur,conn[1][1]['conn_bundle_type_id'],conn[1][1]['conn_type_id'])) 
                                        for conn in enumerate([[j[1],getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)] for j in i['irefs_source']],1) if i['measure']==4]),
 ...
 (:PAR :N N_IN :V {}))\n""".format(str(i['sensor_id']),
                                    ''.join(["""({} -1 (INSIGNALLINK {}) 0)""".format(j[0],j[0]) 
                                            ...
                                            else """({} :SYSTEM "Co-simulation-macro" "{}<--{}" |data_var| {})""".format(j[0],submodel,j[1]['submodel'],import_counter[j[1]['submodel'] if j[1]['cosim'] else submodel]+getCosimCounter(sensor_dec_data,j[1]['iref'],submodel,self.cur,self.dictDB)) 
 ...
 (:PAR :N N_IN :V {}))\n""".format(str(i['sensor_id']),
                                    " ".join(["""({} :SYSTEM "{}" "{}" {})""".format(conn[0], getMacroTypeName(i['source_type'])+'_'+conn[1][0].split('_')[1], getPMT2muxName(self.cur,conn[1][1]['conn_bundle_type_id'],conn[1][0].split('_')[3]), returnVarName(i['measure'])) 
                                        for conn in enumerate([[j,getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)] for j in i['irefs_source']],1) if i['measure'] in (1,2,3)]),
                                    " ".join(["""({} :SYSTEM "{}" "{}" P)""".format(conn[0], getMacroTypeName(i['source_type'])+'_'+conn[1][0].split('_')[1], getMeterName(self.cur,conn[1][1]['conn_bundle_type_id'],conn[1][1]['conn_type_id'])) 
                                        for conn in enumerate([[j,getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)] for j in i['irefs_source']],1) if i['measure']==4]),
                                    "".join([j for j in ['1'] if i['function']==2]),"".join([j for j in ['0'] if i['function']==1]),
                                    str(max(1,len([j for j in i['irefs_source']])))) 
                                    for i in sensor_data if i['function'] in (1,2) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)]))  
 ...
 (:PAR :N N_IN :V {}))\n""".format(str(i['sensor_id']),
                                    ...
                                    "".join([j for j in ['1'] if i['function']==2]),"".join([j for j in ['0'] if i['function']==1]),
                                    getLength(i,submodel)) 
                    for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (1,2) and 
 ...
 (:PAR :N N_IN :V 1))\n""".format(j[1],
                                    " ".join(["""(1 :SYSTEM "{}" "{}" {})""".format(getMacroTypeName(i['source_type'])+'_'+j[1].split('_')[1], getPMT2muxName(self.cur,getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)['conn_bundle_type_id'],j[1].split('_')[3]), returnVarName(i['measure']))]),
                                    " ".join(["""(1 :SYSTEM "{}" "{}" P)""".format(getMacroTypeName(i['source_type'])+'_'+j[1].split('_')[1], getMeterName(self.cur,getConnValuesByFeature(i['source_type'],j[1].split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)['conn_bundle_type_id'],getConnValuesByFeature(i['source_type'],j.split('_')[1],j[1].split('_')[3],self.cur,self.dictDB)['conn_type_id']))])) 
                                    for i in sensor_data if i['function'] ==6 and i['measure'] in (1,2,3,4) for j in i['irefs_source']]))
                                    
                #add Adder comp if function  Individual signals for each target (6) and target!=Custom(1) and source type in (4)
                data.append("".join(["""((:EO :N "Sensor_{}" :T ADDER_CONT)
 (:VAR :N INSIGNAL :B #S(MS-SPARSE DEFAULT-VALUE NIL DIMENSION 1 VALUE ({}))
 (:PAR :N COEFF :DIM (1) :V #(1))
 (:PAR :N N_IN :V 1))\n""".format(j['iref'],"(1 -1 (INSIGNALLINK 1) 0))" if submodel==j['submodel'] else 
 """(1 :SYSTEM "Co-simulation-macro" "{}<--{}" |data_var| {})""".format(submodel,j['submodel'],(import_counter[j['cosim']] if j['cosim'] else import_counter[submodel])+getCosimCounter(sensor_dec_data,j['iref'],submodel,self.cur,self.dictDB)))
                                    for i in sensor_data if i['target']!=1 and i['function']==6 and i['source_type']==4 for j in i['irefs_source'] if
 ...
 (:PAR :N N_IN :V 1))\n""".format(j['iref'],"(1 -1 (INSIGNALLINK 1) 0)" if submodel==supervisory_submodel else 
    """(1 :SYSTEM "Co-simulation-macro" "{}<--{}" |data_var| {})""".format(j['submodel'],supervisory_submodel,import_counter[j['submodel'] if j['network_side'] else submodel]+getCosimCounter(sensor_dec_data,j['iref'],submodel,self.cur,self.dictDB)))
                                    for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                                    j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel'] or 
                                    supervisory_submodel==submodel and supervisory_submodel!=j['submodel'] and not j['network_side']]))

                #add Adder comp if function  Individual signals for each target (6) and target=Custom(1) and target type in (4) for decoupling option at network_side
                data.append("".join(["""((:EO :N "Sensor_{}" :T ADDER_CONT)
 (:VAR :N INSIGNAL :B #S(MS-SPARSE DEFAULT-VALUE NIL DIMENSION 1 VALUE ({})))
 (:PAR :N COEFF :DIM (1) :V #(1))
 (:PAR :N N_IN :V 1))\n""".format(j['iref'],"""(1 :SYSTEM "Co-simulation-macro" "{}<--{}" |data_var| {})""".format(supervisory_submodel,j['submodel'],import_counter[j['submodel']] + getCosimCounter(sensor_dec_data,j['iref'],submodel,self.cur,self.dictDB)))
                                    for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                                    for j in i['irefs_source'] if submodel==supervisory_submodel and submodel!=j['submodel'] and not j['network_side']]))

                #add Adder comp if function  Individual signals for each target (6) and target=Custom(1) and target type in (4) for decoupling option at feature side
                data.append("".join(["""((:EO :N "Sensor_{}" :T ADDER_CONT)
 ...
 (:PAR :N N_IN :V 1))\n""".format(j['iref'])
                ...
                #add Adder comp if function  Same signal for all targets (5) and source type in (4)                   
                data.append("".join(["""((:EO :N "Sensor_{}" :T ADDER_CONT)
 (:VAR :N INSIGNAL :B #S(MS-SPARSE DEFAULT-VALUE NIL DIMENSION 1 VALUE ({})))
 (:PAR :N COEFF :DIM (1) :V #(1))
 (:PAR :N N_IN :V 1))\n""".format(str(i['sensor_id']),
                                    ...
                                    """(1 :SYSTEM "Co-simulation-macro" "{}<--{}" |data_var| {})""".format(submodel,j['submodel'],(import_counter[j['cosim']] if j['cosim'] else import_counter[submodel])+getCosimCounter(sensor_dec_data,j['iref'],submodel,self.cur,self.dictDB))
                                    for j in i['irefs_source']]))
                                    for i in sensor_dec_data if i['function']==5 and i['source_type']==4 and 
                                    ([True for j in i['irefs_source'] if submodel==j['submodel'] and submodel!=j['cosim']] or [True for j in i['irefs_target'] if submodel==j['submodel'] or submodel==j['cosim']])]))
                print('+-----------*****------------')
                print(data)
                data.append('(CONNECTIONS\n')
                #Source connections to sensor if measure==Custom and not Individual signals for each target and source type in (1,2,3) and target type!=4
                data.append(''.join([""" (("Sensor_{}" (INSIGNALLINK {})) "Int_Ref_Sensor_Source_{}" 0 0 NIL)\n""".format(
                    str(i['sensor_id']),str(j[0]),j[1]['iref']) 
                    for i in sensor_dec_data if i['measure']==5 and i['function']!=6 and i['source_type'] in (1,2,3) and i['target_type']!=4 
                    for j in enumerate([j for j in i['irefs_source'] 
                                            if [True for j in i['irefs_target'] if j['submodel']==submodel and not j['network_side'] or submodel==j['cosim'] and j['network_side']]
                                            or j['submodel']==submodel and not j['network_side'] or j['cosim']==submodel and j['network_side']],1)
                        if j[1]['submodel']==submodel and not j[1]['network_side'] or j[1]['cosim']==submodel and j[1]['network_side']]))
                #Source connections to sensor if source type==Supervisory ctrl (4) and ((traget!=Custom (1) and function == Individual signals for each target) or function==Same signal for all targets (5))
                data.append(''.join([""" (("Sensor_{}" (INSIGNALLINK 1)) "Int_Ref_Sensor_Source_{}" 0 0 NIL)\n""".format(
                    ...
                    for i in sensor_dec_data if ((i['target']!=1 and i['function']==6) or i['function']==5) and i['source_type']==4 
                    for j in i['irefs_source'] if submodel==j['submodel']]))
                #Source/target connections to sensor if source type==Supervisory ctrl (4) and function == Individual signals for each target and target submodel != supervisory submodel
                ...
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1],i[1]) 
                                    for i in enumerate([j['iref'] for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                                    j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel'] or 
                ...
                data.append(''.join(["""(CONNECTION-LINE :AT ({}) :FIRST-LINK ("Sensor_{}" (1 0.531) {}) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_{}_{}") :DIR {})\n""".format(
                                    ...
                                    j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel'] or 
                                    supervisory_submodel==submodel and supervisory_submodel!=j['submodel'] and not j['network_side']],1+count_sensors)]))
                count_sensors+=len([j['iref'] for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                                    j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel'] or 
                                    supervisory_submodel==submodel and supervisory_submodel!=j['submodel'] and not j['network_side']])

                                    
                #add Adder comp for target type == supervisory contrl (4) if function (6)  measure in (5) and source type in (1,2,3) for decoupling mode
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1],i[1]) 
                    for i in enumerate([j['iref'] for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                ...
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    ...
                    for i in enumerate([j['iref'] for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                                            for j in i['irefs_source'] if supervisory_submodel==submodel and submodel!=j['submodel'] and not j['network_side']],1+count_sensors)]))
                count_sensors+=len([j['iref'] for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                                            for j in i['irefs_source'] if supervisory_submodel==submodel and submodel!=j['submodel'] and not j['network_side']])

                #add Adder comp for function Average (3), Add (4), Same signal for all targets (5) if measure in (1,2,3,4) and source type in (1,2,3)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['sensor_id'],i[1]['sensor_id']) 
                    for i in enumerate([i for i in sensor_data if i['function'] in (3,4,5) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)],1+count_sensors)]))
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    str(50+35*i[0]),str(50+35*i[0]+j[0]),str(50+35*i[0]+j[0]),j[1][1].split('_')[0],j[1][1]) 
                    for i in enumerate([i for i in sensor_data if i['function'] in (3,4,5) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)],1+count_sensors) for j in enumerate(i['irefs_target'],0)]))
                count_sensors+=len([i for i in sensor_data if i['function'] in (3,4,5) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)])
                
                #add min/max comp for function Min(1),Max(2)) if measure in (1,2,3,4) and source type in (1,2,3)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:minmaxd.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['sensor_id'],i[1]['sensor_id']) 
                    for i in enumerate([i for i in sensor_data if i['function'] in (1,2) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)],1+count_sensors)]))
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    str(50+35*i[0]),str(50+35*i[0]+j[0]),str(50+35*i[0]+j[0]),j[1][1].split('_')[0],j[1][1]) 
                    for i in enumerate([i for i in sensor_data if i['function'] in (1,2) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)],1+count_sensors) for j in enumerate(i['irefs_target'],0)]))
                count_sensors+=len([i for i in sensor_data if i['function'] in (1,2) and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3)])
                

                #add Adder comp for function Individual signals for each target (6) if measure in (1,2,3,4) and source type in (1,2,3) for decoupling mode if network_side
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1][1],i[1][1]) 
                    for i in enumerate([j for i in sensor_data if i['function']==6 and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3) for j in i['irefs_source']],1+count_sensors)]))
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    str(50+35*sensor[0]),str(50+35*sensor[0]),sensor[1][1],sensor[1][1]) 
                    for sensor in enumerate([j for i in sensor_data if i['function']==6 and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3) for j in i['irefs_source']],1+count_sensors)]))
                count_sensors+=len([j for i in sensor_data if i['function']==6 and i['measure'] in (1,2,3,4) and i['source_type'] in (1,2,3) for j in i['irefs_source']])  
                
                #add Adder comp for function Individual signals for each target (6) if measure==5 and source type in (1,2,3) for decoupling mode if feature side
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['iref'],i[1]['iref']) 
                    for i in enumerate([j for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                ...
                data.append(''.join(["""(CONNECTION-LINE :AT ((237/10 {}) (10 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Source_{}") :DIR :LEFT :ARROW (4864 8 8))\n""".format(
                    ...
                    for sensor in enumerate([j for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                    for j in i['irefs_source'] if submodel==j['submodel'] and not j['network_side'] and not supervisory_submodel==submodel],1+count_sensors)]))
                count_sensors+=len([j for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                                    for j in i['irefs_source'] if submodel==j['submodel'] and not j['network_side'] and not supervisory_submodel==submodel])                
                
                #add Min/Max comp if function (1,2) and measure== Custom(5) and source type in (1,2,3)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:minmaxd.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['sensor_id'],i[1]['sensor_id'])
                    for i in enumerate([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (1,2) and 
                       [True for j in i['irefs_source'] 
                        if [True for j in i['irefs_target'] if j['submodel']==submodel and not j['network_side'] or submodel==j['cosim'] and j['network_side']]
                ...
                count_sensors+=len([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (1,2) and 
                        [True for j in i['irefs_source'] 
                            if [True for j in i['irefs_target'] if j['submodel']==submodel and not j['network_side'] or submodel==j['cosim'] and j['network_side']]
                            or j['submodel']==submodel and not j['network_side'] or j['cosim']==submodel and j['network_side']]])      
                
                #Adder comp if function (3,4,5) and measure== Custom(5) and source type in (1,2,3)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['sensor_id'],i[1]['sensor_id'])
                    for i in enumerate([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (3,4,5) and 
                ...
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    ...
                    for i in enumerate([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (3,4,5)],1+count_sensors) for j in enumerate([j for j in i[1]['irefs_target'] if j['submodel']==submodel and not j['network_side'] or j['cosim']==submodel and j['network_side']],0)]))
                data.append(''.join(["""(CONNECTION-LINE :AT ((237/10 {}) (10 {})) :FIRST-LINK ("Sensor_{}" (1 0.491) (INSIGNALLINK {})) :LAST-LINK (:SELF (0.0 0.144) "Int_Ref_Sensor_Source_{}") :DIR :LEFT :ARROW (4864 8 8))\n""".format(
                    str(50+35*i[0]+j[0]),str(50+35*i[0]+j[0]),str(i[1]['sensor_id']),str(j[0]+1),str(j[1]['iref']))
                    for i in enumerate([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (3,4,5)],1+count_sensors) for j in enumerate([j for j in i[1]['irefs_source'] if j['submodel']==submodel and not j['network_side'] or j['cosim']==submodel and j['network_side']],0)]))
                count_sensors+=len([i for i in sensor_dec_data if i['source_type'] in (1,2,3) and i['measure']==5 and i['function'] in (3,4,5) and ([True for j in i['irefs_source'] if submodel==j['submodel'] and submodel!=j['cosim'] and not j['network_side']] or [True for j in i['irefs_target'] if submodel==j['submodel'] or submodel==j['cosim']])])  
                
                #add Adder comp if function Same signal for all targets (5) and source type in (4)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*i[0]),i[1]['sensor_id'],i[1]['sensor_id'])
                    for i in enumerate([i for i in sensor_data if i['function']==5 and i['source_type']==4],1+count_sensors)]))
                data.append(''.join(["""(CONNECTION-LINE :AT ((58 {}) (58 {}) (694 {})) :FIRST-LINK ("Sensor_{}" (1 0.531) OUTSIGNALLINK) :LAST-LINK (:SELF (1.0 0.205) "Int_Ref_Sensor_Target_{}") :DIR :RIGHT :ARROW (19 8 8))\n""".format(
                    str(50+35*i[0]),str(50+35*i[0]+j[0]),str(50+35*i[0]+j[0]),i[1]['sensor_id'],j[1])
                    for i in enumerate([i for i in sensor_dec_data if i['function']==5 and i['source_type']==4 and 
                ...
                count_sensors+=len([i for i in sensor_data if i['function']==5 and i['source_type']==4])   

                #add Adder comp if (function  Individual signals for each target (6) and target!=Custom(1))) and source type in (4)
                data.append(''.join(["""(EQUATION-FRAME :AT ((41 {})) :R (16 16) :ICON "lib:adder.ids" :SLOT ("Sensor_{}") :NAME "Sensor_{}" :PADDING 3 :DATA :EO)\n""".format(
                    str(50+35*sensor[0]),sensor[1],sensor[1])
                    for sensor in enumerate([j for i in sensor_dec_data if i['function']==6 and i['target']!=1 and i['source_type']==4 
                ...
                data.append(''.join(["""(CONNECTION-LINE :AT ((237/10 {}) (10 {})) :FIRST-LINK ("Sensor_{}" (1 0.491) (INSIGNALLINK 1)) :LAST-LINK (:SELF (0.0 0.144) "Int_Ref_Sensor_Source_{}") :DIR :LEFT :ARROW (4864 8 8))\n""".format(
                    str(50+35*sensor[0]),sensor[1],sensor[1])
                    for sensor in enumerate([j for i in sensor_data if i['function']==5 and i['target_type']==4],1+count_sensors) for j in enumerate(i['irefs_target'],0)]))
                count_sensors+=len([j for i in sensor_data if i['function']==6 and i['target']!=1 and i['source_type']==4 for j in i['irefs_source']])                 
                ...
                idm_conn+=''.join(["""\n (("Sensor-macro" "Int_Ref_Sensor_Target_{}") ("{}{}" "Int_Ref_Sensor_Target_{}") 0 0 NIL)""".format(
                    j['iref'],getMacroTypeName(i['target_type']),'_'+j['iref'].split('_')[1] if i['target_type']!=4 else '',j['iref'].split('_')[0] if i['target_type'] in (1,2,3) else j['iref']) 
                ...
                idm_conn+=''.join(["""\n (("Sensor-macro" "Int_Ref_Sensor_Source_{}") ("{}{}" "Int_Ref_Sensor_Source_{}") 0 0 NIL)""".format(
                    j['iref'],getMacroTypeName(i['source_type']),'_'+j['iref'].split('_')[1] if i['source_type']!=4 else '',j['iref'].split('_')[0] if i['source_type']!=4 else j['iref'])
                ...
                #source connections from type supervisory ctrl to sensor if target type ==Customer, Plant or Device (1,2,3) and function== Individual signals for each target (6) and target [5]==custom (1)
                idm_conn+=''.join(["""\n (("Supervisory_control" "Int_Ref_Sensor_Source_{}") ("Sensor-macro" "Int_Ref_Sensor_Source_{}") 0 0 NIL)""".format(j['iref'],j['iref'])
                    for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                    supervisory_submodel==submodel and supervisory_submodel!=j['submodel'] and not j['network_side']])
                #target connections from type supervisory ctrl to sensor if target type ==Customer, Plant or Device (1,2,3) and function== Individual signals for each target (6) and target [5]==custom (1)
                idm_conn+=''.join(["""\n (("Sensor-macro" "Int_Ref_Sensor_Target_{}") ("{}{}" "Int_Ref_Sensor_Target_{}") 0 0 NIL)""".format(j['iref'],getMacroTypeName(i['target_type']),'_'+j['iref'].split('_')[1] if i['target_type']!=4 else '',j['iref'].split('_')[0])
                    for i in sensor_dec_data if i['target']==1 and i['function']==6 and i['source_type']==4 for j in i['irefs_target'] if
                    j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel']])
                                    
                #source connections from type supervisory ctrl to feature if target type Customer, Plant or Device (1,2,3) and function== Individual signals for each target (6) and target [5]==custom (1)
                idm_conn+=''.join(["""\n (("Supervisory_control" "Int_Ref_Sensor_Source_{}") ("{}_{}" "Int_Ref_Sensor_Target_{}") 0 0 NIL)""".format(
                    ...
                    for i in sensor_dec_data if i['source_type']==4 and i['target_type'] in (1,2,3) and i['function']==6 and i['target']==1 for j in i['irefs_target'] if 
                ...
                #connections from feature to supervisory if target type ==4 and source type in (1,2,3) supervisory ctrl (4) and function == Individual signals for each target (6) and target==Custom (1) and measure = Custom (5)
                ...
                #connections from Sensor to supervisory if target type == supervisory ctrl (4) and source type in (1,2,3) and function == Individual signals for each target (6) and target==Custom (1) and measure = Custom (5) for decoupling mode if network side
                idm_conn+=''.join(["""\n (("Sensor-macro" "Int_Ref_Sensor_Target_{}") ("Supervisory_control" "Int_Ref_Sensor_Target_{}") 0 0 NIL)""".format(
                   j['iref'],j['iref']) 
                    for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                    for j in i['irefs_source'] if supervisory_submodel==submodel and submodel!=j['submodel'] and not j['network_side']])
                #connections from Sensor to supervisory if target type == supervisory ctrl (4) and source type in (1,2,3) and function == Individual signals for each target (6) and target==Custom (1) and measure = Custom (5) for decoupling at feature side
                ...
                idm+="""\n((MACRO-OBJECT :N "Sensor-macro" :T ICE-MACRO :ETM 3857526820 :STM 3857526845){}{}{}{}{}{}{}{})""".format(
                    #iref sources if source type Customer, Plant, Device (1,2,3) and measure==Custom (5) and not (function == Individual signals for each target (6) and target [5]==custom (1))
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Source_{}" :T IN :F 208)""".format(j['iref']) 
                        for i in sensor_dec_data if i['measure']==5 and i['source_type'] in [1,2,3] and not (i['function']==6 and i['target']==1) for j in i['irefs_source'] if submodel==j['submodel'] and not j['network_side'] or submodel==j['cosim'] and j['network_side']]),
                    #iref sources if type is supervisory ctrl (4) and function == Same signal for all targets (5)
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Source_{}" :T IN :F 208)""".format(j['iref'])                   
                        for i in sensor_dec_data if i['source_type']==4 and i['function']==5 for j in i['irefs_source'] if supervisory_submodel==submodel]),
                    #iref sources if type is supervisory ctrl (4) and function== Individual signals for each target (6)
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Source_{}" :T IN :F 208)""".format(j['iref']) 
                        for i in sensor_dec_data if i['source_type']==4 and i['function']==6 for j in i['irefs_target'] if supervisory_submodel==submodel and supervisory_submodel!=j['submodel'] and not j['network_side']]),
                    #iref targets if source type is supervisory ctrl (4) and function== Individual signals for each target (6)
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 224)""".format(j['iref']) 
                        for i in sensor_dec_data if i['source_type']==4 and i['function']==6 for j in i['irefs_target'] if j['submodel']==submodel and not j['network_side'] and supervisory_submodel!=j['submodel']]),
                    #iref targets if target type Customer, Plant or Device (1,2,3) and target ==Custom (1) and not (function == Individual signals for each target (6) and measure==custom (5))
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 224)""".format(j['iref']) 
                        for i in sensor_dec_data if i['target']==1 and i['target_type'] in (1,2,3) and not (i['function']==6 and i['measure']==5) for j in i['irefs_target'] if submodel==j['submodel'] and not j['network_side'] or submodel==j['cosim'] and j['network_side']]),
                    #iref targets if target type Supervisory Ctrl (4) and not (function== Individual signals for each target (6) and measure==custom)
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 224)""".format(j['iref']) 
                        for i in sensor_dec_data if i['target_type']==4 and not (i['function']==6 and i['measure']==5) 
                        for j in i['irefs_target'] if submodel==j['submodel'] and not j['network_side'] or submodel==j['cosim'] and j['network_side']]),
                    #iref targets if target type Supervisory Ctrl (4) and function== Individual signals for each target (6) for decoupling at network side
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Target_{}" :T OUT :F 224)""".format(j['iref']) 
                        for i in sensor_dec_data if i['target_type']==4 and i['function']==6
                        for j in i['irefs_source'] if supervisory_submodel==submodel and submodel!=j['submodel'] and not j['network_side']]),
                    #irefs targets if target type==Supervisory ctrl (4) and function == Individual signals for each target (6) for decoupling at feature side
                    ''.join(["""\n (:IREF :N "Int_Ref_Sensor_Source_{}" :T IN :F 208)""".format(j['iref']) 
                        for i in sensor_dec_data if i['target_type'] ==4 and i['source_type'] in(1,2,3) and i['function']==6 and i['target']==1 and i['measure'] ==5 
                        for j in i['irefs_source'] if submodel==j['submodel'] and not j['network_side'] and not supervisory_submodel==submodel]))
...
(close-unused-documents))"""
    ...
    def insertLines (self,submodel,requestedOutputs,modellingSettings,idm,idc,networks):
        ...
        appendToFile("\n\n;;;;insert lines\n",self.plugin_dir+"\\ida_scripts",self.plugin_dir+"\\ida_scripts\\script_components.txt")
        sql="""WITH sub AS(
