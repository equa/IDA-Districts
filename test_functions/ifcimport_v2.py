...
file_shp="C:\\EQUA\\Projekte\\Hellerhöfe\\shp\\FZN-KBN-ARC-03-XXX-GE-18-AB-ARC Modell_abstractBIM_v7.shp"
file_ifc="C:\\EQUA\\Projekte\\Hellerhöfe\\ifc\\FZN-KBN-ARC-03-XXX-GE-18-AB-ARC Modell_abstractBIM.ifc"
#file_shp="C:\\EQUA\\Projekte\\Hellerhöfe\\shp\\FSS-SPA-ARC-03-GES-XX-18-AB-Freigabe LPH3 TGA_abstractBIM.shp"
...
for window in ifc.by_type("IfcWindow"):
    try:
        ...
        win_area = get_quantity_area(window)
        if win_area <= 0:
            continue

        possible_floors = [fe for fe in floor_elevations if fe >= base_z] or [fe for fe in floor_elevations if fe <= base_z]
...
# Process openings
opening_areas_by_floor = {}
seen_opening_positions = set()
...
for opening in ifc.by_type("IfcOpeningElement"):
    try:
        ...
        rounded_xyz = (round(centroid_x, 2), round(centroid_y, 2), round(base_z, 2))
        if rounded_xyz in seen_opening_positions:
            continue
        seen_opening_positions.add(rounded_xyz)

        open_area = get_quantity_area(opening)
        if open_area <= 0:
            continue
...
for floor_elev, val in floor_data.items():
    ...
    opening_area = opening_areas_by_floor.get(floor_elev, 0)
    total_opening_area = window_area + opening_area

    window_ratio = window_area / surface_area if surface_area > 0 else 0
    opening_ratio = total_opening_area / surface_area if surface_area > 0 else 0
    ...
    records.append({
        ...
        "opening_surface_ratio": round(opening_ratio, 3),
        ...
        "total_opening_area": round(total_opening_area, 3)
    })
