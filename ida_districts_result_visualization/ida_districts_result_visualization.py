...
(
    ...
    def getShowOnMapVarsDict(self,dlg):
        ...
        vars['data'],vars['time']['dt'],vars['time']['first_time_var']=self.getShowOnMapData(dlg,vars)
        return vars
    ...
    def getShowOnMapData(self,dlg,vars):
        ...
        if first_time_var:
            dt=getAvergageByMode(vars[first_time_var]['var_function'],self.cur,self.dictDB,vars[first_time_var]['table_name'])
        else:
            dt=False
        print(dt)
        ...
        if dlg.rbtn_simData.isChecked() and dlg.rbtn_lines.isChecked():
            ...
            if float(loadModellingSettings(self.plugin_dir,self.dictDB)['fd_meterPerNode'])>lineSegVisLength:
                self.iface.messageBar().pushMessage("Info", "The line segment length for visualization should be greater than or equal to the segment length in calculation!!", level=Qgis.Info)
                
            srid=loadProjectConfig(self.plugin_dir,self.dictDB['projectName'])['srid']
            
            if dlg.lineSegVis.text()!='0':
...
(
	...
	CONSTRAINT line_s_{}_vis_pkey PRIMARY KEY (id)
                    ...
                    sql+="""\nINSERT INTO {}.line_s_{}_vis (fid,time,{},geom)
    SELECT s.fid,time,avg(s.{}) AS value,seg.geom
        FROM {}.line_s_{} s, {}.line_seg seg
        WHERE ST_dwithin(ST_LineSubstring(seg.geom,0.01,0.99),s.geom,0.001) and s.fid=seg.lid
        GROUP BY time,s.fid,seg.geom;""".format(self.dictDB['versionName'],vars['color']['name'],vars['color']['name'].split('$')[0],vars['color']['name'].split('$')[0],self.dictDB['versionName'],vars['color']['name'],self.dictDB['versionName'])
                    print(sql)
                    self.cur.execute(sql)
...
(
	...
	CONSTRAINT line_s_{}_vis_pkey PRIMARY KEY (id)
                    ...
                    sql+="""\nINSERT INTO {}.line_s_{}_vis (lid,time,{},geom)
    SELECT s.fid,time,avg(s.{}) AS value,seg.geom
        FROM {}.line_s_{} s, {}.line_seg seg
        WHERE ST_dwithin(ST_LineSubstring(seg.geom,0.01,0.99),s.geom,0.001) and s.fid=seg.lid
        GROUP BY time,s.fid,seg.geom;""".format(self.dictDB['versionName'],vars['size']['name'],vars['size']['name'],vars['size']['name'],self.dictDB['versionName'],vars['size']['name'],self.dictDB['versionName'])
                    print(sql)
                    self.cur.execute(sql)
    ...
    ORDER BY {} {}.{};""".format(first_group,'fid' if vars[first_group]['mode']=='var' else 'id',first_group,
                ...
                ' AND '.join([i for i in #where
                    [' AND '.join([first_group+('.fid=' if vars[first_group]['mode']=='var' else '.id=')+var+('.fid' if vars[var]['mode']=='var' else '.id') 
                        for var in vars if var not in ['time','data']+([first_time_var] if first_time_var else []) and vars[var]['mode']]),
                ...
                ','.join([i for i in #group by
                    ...
                    ','.join([var+'.'+(var if vars[var]['mode']=='var' and vars[var]['var_function']!='Values' else '"'+vars[var]['name'].split('$')[0]+'"') for var in vars if var not in ['time','data'] and (vars[var]['mode'] and vars[var]['var_function'] not in dlg.time_values or vars[var]['var_function']=='Values')])] if i]),
  ...
  FROM (SELECT time FROM a.line_s_t$1 GROUP BY time) a
  GROUP BY time 
  order by time;"""
  
        return [data,dt,first_time_var]
...
FROM {}.{}
WHERE time <= '{}'
ORDER BY fid,time DESC""".format(var['name'].split('$')[0],type,self.dictDB['versionName'],var['table_name'],time['endtime'])
        elif var['var_function']=='Sum':
             sql="""SELECT fid,ST_asText(geom) AS geom,sum({}) AS {} FROM {}.{} WHERE time BETWEEN '{}' AND '{}' GROUP BY fid,ST_asText(geom) ORDER BY fid""".format(var['name'].split('$')[0],type,self.dictDB['versionName'],var['table_name'],time['starttime'],time['endtime'])
        elif var['var_function']=='First value':
            sql="""SELECT DISTINCT ON (fid) 
    fid,ST_asText(geom) AS geom,{} AS {},time
FROM {}.{}
WHERE time >= '{}'
ORDER BY fid,time ASC""".format(var['name'].split('$')[0],type,self.dictDB['versionName'],var['table_name'],time['starttime'])
        return sql
    ...
    def showOnMapMemoryLayer(self,vars,dlg):            
        ...
        temp_layer = QgsVectorLayer("{}?crs=epsg:{}&field=id:integer&{}{}".format(
            ...
            '&'.join(['field={}:numeric'.format(col) for col in column_names])),dlg.layer_name.text(), "memory")
        ...
        if vars['time']['first_time_var']:
            ...
            temporalNavigationObject.setNavigationMode(1)
            ...
            if vars['time']['dt']=='hour':
                ...
                dt=datetime.timedelta(days=30.4166667)
            else:
                dt=datetime.timedelta(seconds=int(vars['time']['dt']))
            print(dt)
            print(dt+getDatetimeFromString(vars['time']['endtime']))
            temporalNavigationObject.setTemporalExtents(QgsDateTimeRange(getDatetimeFromString(vars['time']['starttime']), dt+getDatetimeFromString(vars['time']['endtime'])))
            
            temporalNavigationObject.setLooping(True)
            temporalNavigationObject.setAnimationState(2)
            ...
            if vars['time']['dt']=='hour':
                ...
                interval.setSeconds(int(vars['time']['dt']))
        ...
        if vars['color']['mode']:
            print('-----color------')
            target_field = vars['color']['name'].split('$')[0]
            print(target_field)
        ...
        if vars['size']['mode'] or vars['rotation']['mode']:
            ...
            if dlg.checkbox_varSize.isChecked():
                ...
                scale="""coalesce(scale_exp("{}", {}, {}, {}, {}, 0.57), 0)""".format(vars['size']['name'],min_value,max_value,dlg.size_symbolMin.text(),dlg.size_symbolMax.text())
                print(scale)
                symbol.symbolLayer(0).setDataDefinedProperty(QgsSymbolLayer.PropertySize,QgsProperty.fromExpression(scale))
            ...
            if dlg.checkbox_varRotation.isChecked():
                ...
                rotation="""coalesce(scale_exp("{}", {}, {}, {}, {}, 0.57), 0)""".format(vars['rotation']['name'],min_value,max_value,dlg.rotation_symbolMin.text(),dlg.rotation_symbolMax.text())
                print(rotation)
